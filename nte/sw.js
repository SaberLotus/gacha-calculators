...
self.addEventListener('activate', e => e.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k !== CACHE && !k.startsWith(CACHE + '-fonts')).map(k => caches.delete(k)))).then(() => self.clients.claim())));
